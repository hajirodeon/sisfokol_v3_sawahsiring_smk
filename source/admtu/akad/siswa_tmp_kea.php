...
if (empty($tapelkd))
	...
	if ($count != 0)
		...
		do
			...
			<td>
			<a href="siswa_pdf.php?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&keahkd='.$kd_keahlian.'&swkd='.$kd.'"><img src="'.$sumber.'/img/pdf.gif" width="16" height="16" border="0"></a>
			</td>
		...
		do
			...
			$pro_bid = balikin($rowprox['bidang']);
			$pro_prog = balikin($rowprox['program']);

			echo '<option value="'.$pro_kd.'">'.$pro_bid.' - '.$pro_prog.'</option>';
			}
