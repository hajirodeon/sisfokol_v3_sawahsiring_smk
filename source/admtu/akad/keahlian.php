...
if ($s == "edit")
	...
	$bidang = balikin($rowx['bidang']);
	$program = balikin($rowx['program']);
	}
...
if ($_POST['btnSMP'])
	...
	$s = nosql($_POST['s']);
	$kd = nosql($_POST['kd']);
	$bidang = cegah($_POST['bidang']);
	$program = cegah($_POST['program']);
	...
	if ((empty($bidang)) OR (empty($program)))
		...
		if ($tcc != 0)
			...
			//re-direct
			$pesan = "Keahlian : $bidang - $program, Sudah Ada. Silahkan Ganti Yang Lain...!!";
			pekem($pesan,$filenya);
			exit();
			}
		else
...
Bidang : 
<br>
<input name="bidang" type="text" value="'.$bidang.'" size="30">
<br>
<br>
...
if ($total != 0)
	...
	<td width="1%">&nbsp;</td>
	<td width="1%">&nbsp;</td>
	<td><strong><font color="'.$warnatext.'">Keahlian</font></strong></td>
	</tr>';
	...
	do 
		...
		$nomer = $nomer + 1;
		$i_kd = nosql($row['kd']);
		$i_bidang = balikin2($row['bidang']);
		$i_program = balikin2($row['program']);
		...
		<td width="90%">'.$i_bidang.' - '.$i_program.'</td>
        </tr>';				
		} 
