...
if ($_POST['btnSMP'])
	...
	do
		...
		if ($tcc != 0)
			{
			//update
			mysql_query("UPDATE inv_brg_keahlian SET jml = '$yuhux' ".
							"WHERE kd_brg = '$brgkd' ".
							"AND kd_keahlian = '$kea2x'");
			}
			...
			//insert
			$xyz = md5("$x$i");
			mysql_query("INSERT INTO inv_brg_keahlian(kd, kd_brg, kd_keahlian, jml) VALUES ".
...
if ($s == "edit")
	...
	do
		...
		$kea_bid = balikin2($rkea['bidang']);
		$kea_prog = balikin2($rkea['program']);
		
		
		
		//jml-nya...
		$qkeb = mysql_query("SELECT * FROM inv_brg_keahlian ".
								"WHERE kd_brg = '$brgkd' ".
								"AND kd_keahlian = '$kea_kd'");
		...
		<strong>'.$kea_bid.' - '.$kea_prog.'</strong>
		<br>
		Jumlah :   
