...
if ($_POST['btnSMP'])
	...
	//re-direct
	$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&keahkd=$keahkd&rukd=$rukd&smtkd=$smtkd&skkd=$skkd&swkd=$swkd&&progkd=$progkd";
	xloc($ke);
	exit();
	}
...
if ($_POST['btnSMP2'])
	...
	if (empty($ekstra))
		...
		$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&keahkd=$keahkd&rukd=$rukd&smtkd=$smtkd&skkd=$skkd&swkd=$swkd&";
		pekem($pesan,$ke);
		exit();
		}
	else
...
if (empty($smtkd))
	...
	echo '<option value="'.$stdx_kd.'" selected>'.$jnsx_jenis.' --> '.$stdx_pel.'</option>';
	...
	do
		...
		echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&keahkd='.$keahkd.'&rukd='.$rukd.'&smtkd='.$smtkd.'&skkd='.$skkd.'&swkd='.$swkd.'&progkd='.$std_kd.'">'.$jns_jenis.' --> '.$std_pel.'</option>';
		}
