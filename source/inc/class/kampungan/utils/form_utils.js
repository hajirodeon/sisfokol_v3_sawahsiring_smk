...
function getColorPickerHTML(id, target_form_element) {
	...
	html += '<img id="' + id + '" src="../../themes/' + kampungan.getParam("theme") + '/images/color.gif"';
	...
	html += ' width="20" height="16" border="0" title="' + kampungan.getLang('lang_browse') + '"';
	html += ' class="mceButtonNormal" alt="' + kampungan.getLang('lang_browse') + '" /></a>';
...
function pickColor(e, target_form_element) {
	if ((e.keyCode == 32 || e.keyCode == 13) || e.type == "mousedown")
		kampunganPopup.pickColor(e, target_form_element);
}
...
function setBrowserDisabled(id, state) {
	...
	if (lnk) {
		if (state) {
			...
			kampungan.switchClass(img, 'mceButtonDisabled', true);
		} else {
			lnk.setAttribute("href", lnk.getAttribute("realhref"));
			kampungan.switchClass(img, 'mceButtonNormal', false);
		}
	}
}
...
function getBrowserHTML(id, target_form_element, type, prefix) {
	...
	html += '<img id="' + id + '" src="../../themes/' + kampungan.getParam("theme") + '/images/browse.gif"';
	html += ' onmouseover="this.className=\'mceButtonOver\';"';
	html += ' onmouseout="this.className=\'mceButtonNormal\';"';
	html += ' onmousedown="this.className=\'mceButtonDown\';"';
	html += ' width="20" height="18" border="0" title="' + kampungan.getLang('lang_browse') + '"';
	html += ' class="mceButtonNormal" alt="' + kampungan.getLang('lang_browse') + '" /></a>';
...
function openBrower(img_id, target_form_element, type, option) {
	...
	if (img.className != "mceButtonDisabled")
		kampunganPopup.openBrowser(target_form_element, type, option);
}
...
function addClassesToList(list_id, specific_option) {
	...
	var styles = kampungan.getParam('theme_advanced_styles', false);
	styles = kampungan.getParam(specific_option, styles);
	...
	if (styles) {
		...
		var csses = kampungan.getCSSClasses(kampungan.getWindowArg('editor_id'));
...
function getStyle(elm, attrib, style) {
	var val = kampungan.getAttrib(elm, attrib);
