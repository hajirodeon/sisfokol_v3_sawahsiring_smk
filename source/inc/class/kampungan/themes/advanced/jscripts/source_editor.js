function saveContent() {
	kampungan.setContent(document.getElementById('htmlSource').value);
...
/*	html = html.replace(new RegExp('<(p|hr|table|tr|td|ol|ul|object|embed|li|blockquote)', 'gi'),'\n<$1');
	...
	html = kampungan.regexpReplace(html, '<br />','<br />\n','gi');
	html = kampungan.regexpReplace(html, '\n\n','\n','gi');*/
	return html;
}
...
function onLoadInit() {
	...
	document.forms[0].htmlSource.value = fixContent(kampungan.getContent(kampungan.getWindowArg('editor_id')));
	resizeInputs();

	if (kampungan.getParam("theme_advanced_source_editor_wrap", true)) {
		setWrap('soft');
...
function renderWordWrap() {
	if (kampungan.isMSIE || kampungan.isGecko)
