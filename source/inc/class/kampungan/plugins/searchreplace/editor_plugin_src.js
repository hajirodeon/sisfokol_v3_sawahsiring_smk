...
kampungan.importPluginLanguagePack('searchreplace', 'en,tr,sv,zh_cn,fa,fr_ca,fr,de,pl,pt_br,cs,nl,da,he,nb,hu,ru,ru_KOI8-R,ru_UTF-8,nn,fi,cy,es,is,zh_tw,zh_tw_utf8,sk');

var kampungan_SearchReplacePlugin = {
	getInfo : function() {
		return {
			longname : 'Search/Replace',
			author : 'Moxiecode Systems',
			authorurl : 'http://kampungan.moxiecode.com',
			infourl : 'http://kampungan.moxiecode.com/kampungan/docs/plugin_searchreplace.html',
	...
	getControlHTML : function(cn)	{
		switch (cn) {
			case "search":
				return kampungan.getButtonHTML(cn, 'lang_searchreplace_search_desc', '{$pluginurl}/images/search.gif', 'mceSearch', true);
			case "replace":
				return kampungan.getButtonHTML(cn, 'lang_searchreplace_replace_desc', '{$pluginurl}/images/replace.gif', 'mceSearchReplace', true);
		}
		return "";
	},
			...
			case "mceSearch":
				if (user_interface) {
					...
					if (value['replacestring'] != null) {
						...
						template['width'] = 320;
						template['height'] = 100 + (kampungan.isNS7 ? 20 : 0);
						template['width'] += kampungan.getLang('lang_searchreplace_replace_delta_width', 0);
						template['height'] += kampungan.getLang('lang_searchreplace_replace_delta_height', 0);
					} else {
						template['file'] = '../../plugins/searchreplace/search.htm'; // Relative to theme
						template['width'] = 310;
						template['height'] = 105 + (kampungan.isNS7 ? 25 : 0);
						template['width'] += kampungan.getLang('lang_searchreplace_search_delta_width', 0);
						template['height'] += kampungan.getLang('lang_searchreplace_replace_delta_height', 0);
					}
					...
					kampungan.openWindow(template, value);
				} else {
					var win = kampungan.getInstanceById(editor_id).contentWindow;
					var doc = kampungan.getInstanceById(editor_id).contentWindow.document;
					var body = kampungan.getInstanceById(editor_id).contentWindow.document.body;

					// Whats the point
					if (body.innerHTML == "") {
						alert(kampungan.getLang('lang_searchreplace_notfound'));
						return true;
					}
					...
					if (value['replacemode'] == "current") {
						...
						kampungan.execInstanceCommand(editor_id, 'mceSearch', user_interface, value, false);

						return true;
					}

					if (kampungan.isMSIE) {
						var rng = kampungan.lastSearchRng ? kampungan.lastSearchRng : doc.selection.createRange();
						var flags = 0;
						...
						if (value['replacemode'] == "all") {
							...
							alert(kampungan.getLang('lang_searchreplace_allreplaced'));
							return true;
						}
...
kampungan.addPlugin("searchreplace", kampungan_SearchReplacePlugin);
