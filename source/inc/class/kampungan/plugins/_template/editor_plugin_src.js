...
kampungan.importPluginLanguagePack('template', 'en,tr,he,nb,ru,ru_KOI8-R,ru_UTF-8,nn,fi,cy,es,is,pl'); // <- Add a comma separated list of all supported languages
...
var kampungan_TemplatePlugin = {
	...
	/**
	 * Gets executed when a kampungan editor instance is initialized.
	 *
	 * @param {kampungan_Control} Initialized kampungan editor control instance. 
	 */
	initInstance : function(inst) {
		// You can take out plugin specific parameters
		alert("Initialization parameter:" + kampungan.getParam("template_someparam", false));
	 ...
	 * A control can be a button, select list or any other HTML item to present in the kampungan user interface.
	...
	getControlHTML : function(cn) {
		switch (cn) {
			case "template":
				return kampungan.getButtonHTML(cn, 'lang_template_desc', '{$pluginurl}/images/template.gif', 'mceTemplate', true);
		}
	 ...
	 *
	 * @param {string} editor_id kampungan editor instance id that issued the command.
		...
		switch (command) {
			...
			case "mceTemplate":
				...
				if (user_interface) {
					...
					kampungan.openWindow(template, {editor_id : editor_id, some_custom_arg : "somecustomdata"});

					// Let kampungan know that something was modified
	...
	/**
	 * Gets called ones the cursor/selection in a kampungan instance changes. This is useful to enable/disable
	 ...
	 *
	 * @param {string} editor_id kampungan editor instance id that was changed.
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		if (node.parentNode.nodeName == "STRONG" || node.parentNode.nodeName == "B") {
			kampungan.switchClass(editor_id + '_template', 'mceButtonSelected');
			return true;
		}

		// Deselect template button
		kampungan.switchClass(editor_id + '_template', 'mceButtonNormal');
	},

	/**
	 * Gets called when a kampungan editor instance gets filled with content on startup.
	 *
	 * @param {string} editor_id kampungan editor instance id that was filled with content.
	...
	/**
	 * Gets called when the contents of a kampungan area is modified, in other words when a undo level is
	 * added.
	 *
	 * @param {kampungan_Control} inst kampungan editor area control instance that got modified.
	 */
	onChange : function(inst) {
	},

	/**
	 * Gets called when kampungan handles events such as keydown, mousedown etc. kampungan
	...
	/**
	 * Gets called when HTML contents is inserted or retrived from a kampungan editor instance.
	 ...
	 * @param {kampungan_Control} inst kampungan editor instance control that performes the cleanup.
	 ...
	 * The prefix is needed so they doesn't collide with future kampungan callback functions.
	 *
...
// Adds the plugin class to the list of available kampungan plugins
kampungan.addPlugin("template", kampungan_TemplatePlugin);
