...
function init() {
	...
	ce.style.cssText = kampunganPopup.getWindowArg('style_text');
	...
	kampungan_EditableSelects.init();
	setupFormData();
...
function applyAction() {
	...
	kampunganPopup.execCommand('mceSetElementStyle', false, kampungan.serializeStyle(kampungan.parseStyle(ce.style.cssText)));
}
...
function generateCSS() {
	...
	ce.style.cssText = kampungan.serializeStyle(kampungan.parseStyle(ce.style.cssText));
}
	...
	for (i=0; i<f.length; i++) {
		for (a=0; a<f[i].elements.length; a++) {
			if (f[i].elements[a].disabled)
				kampungan.addCSSClass(f[i].elements[a], "disabled");
			else
				kampungan.removeCSSClass(f[i].elements[a], "disabled");
		}
	}
}
...
function fillSelect(f, s, param, dval, sep, em) {
	...
	ar = kampungan.getParam(param, dval).split(sep);
