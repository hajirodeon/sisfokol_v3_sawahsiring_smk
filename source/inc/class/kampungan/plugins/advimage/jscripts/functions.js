...
function init() {
	...
	if (action == "update") {
		var src = kampungan.getAttrib(elm, 'src');
		var onmouseoversrc = getImageSrc(kampungan.cleanupEventStr(kampungan.getAttrib(elm, 'onmouseover')));
		var onmouseoutsrc = getImageSrc(kampungan.cleanupEventStr(kampungan.getAttrib(elm, 'onmouseout')));

		src = convertURL(src, elm, true);

		// Use mce_src if found
		var mceRealSrc = kampungan.getAttrib(elm, 'mce_src');
		if (mceRealSrc != "") {
			src = mceRealSrc;
		...
		if (onmouseoversrc != "" && kampungan.getParam('convert_urls'))
			onmouseoversrc = convertURL(onmouseoversrc, elm, true);

		if (onmouseoutsrc != "" && kampungan.getParam('convert_urls'))
			onmouseoutsrc = convertURL(onmouseoutsrc, elm, true);

		// Setup form data
		var style = kampungan.parseStyle(kampungan.getAttrib(elm, "style"));
		...
		formObj.alt.value    = kampungan.getAttrib(elm, 'alt');
		formObj.title.value  = kampungan.getAttrib(elm, 'title');
		formObj.border.value = trimSize(getStyle(elm, 'border', 'borderWidth'));
		formObj.vspace.value = kampungan.getAttrib(elm, 'vspace');
		formObj.hspace.value = kampungan.getAttrib(elm, 'hspace');
		...
		formObj.id.value  = kampungan.getAttrib(elm, 'id');
		formObj.dir.value  = kampungan.getAttrib(elm, 'dir');
		formObj.lang.value  = kampungan.getAttrib(elm, 'lang');
		formObj.longdesc.value  = kampungan.getAttrib(elm, 'longdesc');
		formObj.usemap.value  = kampungan.getAttrib(elm, 'usemap');
		formObj.style.value  = kampungan.serializeStyle(style);
		...
		selectByValue(formObj, 'classlist', kampungan.getAttrib(elm, 'class'));
	...
	if (kampungan.getParam("advimage_constrain_proportions", true))
...
function insertAction() {
	...
	if (kampungan.getParam("accessibility_warnings")) {
		if (formObj.alt.value == "") {
			var answer = confirm(kampungan.getLang('lang_advimage_missing_alt', '', true));
			if (answer == true) {
				formObj.alt.value = " ";
			}
		} else {
	...
	if (onmouseoversrc && onmouseoversrc != "")
		onmouseoversrc = "this.src='" + convertURL(onmouseoversrc, kampungan.imgElement) + "';";

	if (onmouseoutsrc && onmouseoutsrc != "")
		onmouseoutsrc = "this.src='" + convertURL(onmouseoutsrc, kampungan.imgElement) + "';";

	if (elm != null && elm.nodeName == "IMG") {
		setAttrib(elm, 'src', convertURL(src, kampungan.imgElement));
		...
		//kampunganPopup.execCommand("mceRepaint");
		...
		html += makeAttrib('src', convertURL(src, kampungan.imgElement));
...
function updateStyle() {
	...
	if (kampungan.getParam('inline_styles', false)) {
...
function showPreviewImage(src, start) {
	...
	var src = src == "" ? src : kampungan.convertRelativeToAbsoluteURL(kampungan.settings['base_href'], src);

	if (!start && kampungan.getParam("advimage_update_dimensions_onchange", true))
		resetImageData();
...
function getImageData(src) {
	...
	kampungan.addEvent(preloadImg, "error", resetImageData);
...
function getImageListHTML(elm_id, target_form_element, onchange_func) {
	...
	html += ' class="mceImageList" onfocus="kampungan.addSelectAccessibility(event, this, window);" onchange="this.form.' + target_form_element + '.value=';
	...
	for (var i=0; i<kampunganImageList.length; i++)
		html += '<option value="' + kampunganImageList[i][1] + '">' + kampunganImageList[i][0] + '</option>';
